...
5. Выполните запрос разрешений на показ push-уведомлений ([Notification Runtime Permission](https://developer.android.com/develop/ui/views/notifications/notification-permission)) в приложении для Android 13 и выше.
<br/>Для этого вы можете воспользоваться классом [PushPermissionHelper.kt](/sabycom/src/main/java/ru/tensor/sabycom/push/PushPermissionHelper.kt) или реализовать функционал самостоятельно.

6. Установите иконку для отображения в push-уведомлении. Иконка должна быть монохромной и соответствовать [стандартам](https://material.io/design/platform-guidance/android-notifications.html#anatomy-of-a-notification) .
...
7. Вы так же можете установить какой цвет будет использоваться в push-уведомлении. По умолчанию берется атрибут `colorPrimary` из темы вашего приложения.
...
8. Чтобы удалить информацию о пользователе и отписаться от уведомлений, вызовите функцию
```kotlin
Sabycom.logout()
```
