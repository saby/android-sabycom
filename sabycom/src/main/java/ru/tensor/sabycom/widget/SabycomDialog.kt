...
internal class SabycomDialog : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        binding = SabycomDialogBinding.inflate(inflater).apply {
            ...
            viewModel.internetAvailable.observe(viewLifecycleOwner) {
                evaluateJavaScript(getOfflineModeJs(!it, channel))
            }
            ...
            viewModel.openChatEvent.observe(viewLifecycleOwner) { event ->
                event.handleEvent {
                    evaluateJavaScript(openChat(it, channel))
                }
            }
        }
        return binding!!.root
    ...
    override fun onResume() {
        ...
        binding?.apply {
            webView.onResume()
            evaluateJavaScript(getBeforeShowActionJs(channel))
        }
    }
    ...
    private fun prepareWebView(webView: AdvancedWebView) {
        ...
        webView.addJavascriptInterface(
            JSInterface(
                Sabycom.countController,
                onCloseListener = { viewModel.hide() },
                onContentScrollListener = { isContentScrolling = it },
                onRegisterWindowListener = { viewModel.showWebView() },
                onStartPageListener = { viewModel.onPageStarted() }
            ),
            "mobileParent"
        )
    }
    ...
    private fun openChat(chatId: String, channel: String): String {
        return "window.postMessage('{\"action\":\"openChat\",\"value\":\"$chatId\",\"channel\":\"$channel\", \"windowId\":\"chat\"}','*');"
    }
    ...
    private fun SabycomDialogBinding.evaluateJavaScript(script: String) {
