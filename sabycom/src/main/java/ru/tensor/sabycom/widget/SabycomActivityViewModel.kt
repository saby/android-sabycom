...
import ru.tensor.sabycom.data.Event
...
internal class SabycomActivityViewModel(
    ...
    private val repository: Repository = Sabycom.repository,
    ...
    /**
     * Выполняется в момент готовности страницы.
     */
    fun onPageStarted() {
        repository.getChatIdAndForget()?.let {
            _openChatEvent.postValue(Event(it))
        }
    }
